...
def find_predictions_file(run_dir: Path) -> Path | None:
    matches = list(run_dir.glob("predictions_*.xlsx"))
    if not matches:
        logger.warning(f"No predictions Excel in {run_dir}")
        return None
    return matches[0]
...
async def main():
    ...
    if pred_file is None:
        logger.error(
            ...
            "This usually means Phase 1 produced 0 predictions or openpyxl "
            "was not installed when Phase 1 ran. Re-run Phase 1 first."
        )
        return
