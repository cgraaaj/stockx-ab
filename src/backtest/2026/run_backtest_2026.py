...
BACKTEST_CONFIG = {
    "name": "q1_2026_pct_sl",
    "description": (
        "Q1 2026 backtest (Jan-Mar 9): VWAP bias filter, 1% percentage SL, "
    ...
    "batch_size": 20,  # API URL length limit prevents batch_size=50
...
async def run():
    ...
    # Hard filter: drop any prediction after signal_cutoff_time
    cutoff_time = BACKTEST_CONFIG.get("signal_cutoff_time")
    if cutoff_time:
        predictions.call = _filter_by_cutoff(predictions.call, cutoff_time)
        predictions.put = _filter_by_cutoff(predictions.put, cutoff_time)
        after_cutoff = sum(len(e.get("stock_data", [])) for e in predictions.call) + \
                       sum(len(e.get("stock_data", [])) for e in predictions.put)
        logger.info(f"Signal cutoff filter ({cutoff_time}): kept {after_cutoff} predictions")

    from src.analysis.views.export_view import ExportView
    export = ExportView()
    excel_fn = str(run_dir / f"predictions_{BACKTEST_CONFIG['name']}.xlsx")
    export.export_predictions_to_excel(predictions, excel_fn)
...
# ======================================================================
# Helpers for signal cutoff + market context filter
# ======================================================================

def _filter_by_cutoff(date_entries, cutoff_time_str):
    """Remove predictions whose signal time exceeds the cutoff (e.g. 09:45:00)."""
