...
.. deprecated::
    This script is superseded by the unified pipeline. Use instead:
        python -m src.backtest.cli --config q1_2026.json --phase 3b
...
Usage:
  python -m src.backtest.2026.run_gradeA_10l_2026 [results_dir]
"""
...
MAX_CAPITAL_PER_TRADE = 5_00_000      # Max 5L per trade
...
def main():
    ...
    # ── Simulate with 10L capital, max 5L/trade, same-day aware ──────
    capital = float(STARTING_CAPITAL)
    rows = []

    # Pre-simulate all trades (filter + simulate) to get results
    trade_queue = []
    for t in prepared:
    ...
    # Group by date to handle concurrent trades
    from itertools import groupby
    for date_key, day_group in groupby(trade_queue, key=lambda x: x["trade"]["trade_date"]):
        day_trades = list(day_group)
        day_capital_pool = capital          # available capital at start of day
        day_locked = 0.0                    # capital locked in active trades today
        day_results = []                    # (row_dict, rupee_pnl) to apply after day
        ...
        for item in day_trades:
            ...
            if available <= 0:
                print(f"  SKIP {t['trade_date']} | {t['stock']:15s} | "
                      f"No capital left (locked Rs {day_locked:,.0f} of Rs {day_capital_pool:,.0f})")
                continue
        ...
        # End of day: apply all P&L, update capital
        for row, rupee_pnl in day_results:
            ...
            print(
                f"  #{t_no:>2} {row['trade_date']} | {row['stock']:15s} | "
    ...
    print(f"\n{'=' * 80}")
    print(f"  GRADE A+B | 10% SL / 30% Target | 10L Capital | Max 5L/trade | 15min buffer")
    print(f"{'=' * 80}")
    ...
    # ── Write CSV ─────────────────────────────────────────────────────
    csv_path = run_dir / "gradeAB_10l_max5l_15minbuf_trades.csv"
    if rows:
    ...
    json_path = run_dir / "gradeAB_10l_max5l_15minbuf_summary.json"
